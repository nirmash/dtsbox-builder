...
Two reference projects, identical to what `/dtsbox-builder` would generate for these patterns.
Both ship with **smoke-test stubs** so they run locally against the DTS emulator (Docker) without
Azure. Each example was actually executed before being checked in — see commits `e3d8695`
(fan-out) and `7aa881e` (chaining) for the dogfood verifications.
...
cd examples/square_numbers     # or examples/pipeline_demo
...
dtsbox run square_all --input '[3,5,7]'        # for square_numbers
dtsbox run process_pipeline --input '5'        # for pipeline_demo
